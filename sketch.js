...
function draw() {
  ...
  randomSeed(1);
  for (let i=0; i<250; i++)
    point(random()*canvasWidth,random()*canvasHeight);
  stroke('black');
  ...
  boids.forEach(boid => {
    boid.display();
    qt.insert({
      x: boid.position.x - boid.diameter,
      y: boid.position.y - boid.diameter,
      width: boid.diameter * 2,
      height: boid.diameter * 2,
      b: boid
    });
  });
  moveBoids();
